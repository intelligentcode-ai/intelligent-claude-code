...
#### `enforcement.block_all_capitals_filenames`

**Type**: `boolean`
**Default**: `false`
**Description**: Controls whether files with all-capitals names are blocked.

**Values**:
- `false` (default): Warns about all-capitals filenames but allows file creation with lowercase suggestion
- `true`: Blocks all-capitals filenames entirely until renamed to lowercase

**Example**:
```json
{
  "enforcement": {
    "block_all_capitals_filenames": true
  }
}
```

**Behavior**:
- Detects files like `UPLOAD_LIMIT_FIX_SUMMARY.md`
- When `false`: Shows warning: "⚠️ WARNING: Filename is ALL-CAPITALS - use lowercase for consistency"
- When `true`: Blocks file creation with error: "🚫 ALL-CAPITALS filenames are blocked by configuration"
- Applies to both Write tool and Bash file creation (heredoc, echo redirect, tee)

**Use Cases**:
- Enable for teams requiring strict naming conventions
- Keep disabled (default) for flexibility with automated tools
- Helps maintain consistent lowercase naming in projects
